...
import { useLinkedAccounts, useUnlinkOAuth } from "@/lib/hooks/use-linked-accounts";
import DiscordLinkModal from "@/components/settings/DiscordLinkModal";
import IconDiscordShared from "@/components/icons/IconDiscord";
...
function IconDiscord({ size = 16, color = C.muted }: { size?: number; color?: string }) {
  return <IconDiscordShared size={size} color={color} />;
}
...
// ── Cuentas vinculadas card ──
function LinkedAccountsCard({ onOpenDiscordModal }: { onOpenDiscordModal: () => void }) {
  ...
  function handleUnlinkDiscord() {
    if (!discord) return;
    const ok = typeof window !== "undefined"
      ? window.confirm("¿Desvincular tu cuenta de Discord? El bot dejará de reconocerte.")
      : true;
    if (!ok) return;
    unlink.mutate(
      { provider: "DISCORD" },
      {
        onSuccess: () => toast.success("Discord desvinculado"),
        onError: (err) => {
          const msg = err instanceof Error ? err.message : "No se pudo desvincular";
          toast.danger("No se pudo desvincular", { description: msg });
        },
      },
    );
  }
  ...
  return (
    <SCard>
      <Row
        ...
        label={discord ? `Discord · ${discord.provider_display_name}` : "Discord"}
        right={
          isLoading ? (
            ...
            <button
              ...
              style={{
                ...
                padding: "4px 10px",
                borderRadius: 999,
                cursor: unlink.isPending ? "not-allowed" : "pointer",
                opacity: unlink.isPending ? 0.5 : 1,
              }}
            >
              {unlink.isPending ? "..." : "Desvincular"}
            </button>
          ) : (
            <div style={{ display: "flex", gap: 6 }}>
              <a
                href={`${API_ORIGIN}/api/v1/auth/oauth/discord/start?mode=link&returnTo=/config`}
                style={{
                  fontSize: 12,
                  fontWeight: 600,
                  color: C.text,
      ...
      <Row
        ...
        label={google ? `Google · ${google.provider_display_name}` : "Google"}
