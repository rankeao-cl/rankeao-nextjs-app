...
function normalizeCard(data: any): CardView | null {
    ...
    if (data.scryfall_card && data.scryfall_card.name) {
        ...
        return {
            ...
            rarity: sc.rarity || "", set_name: sc.set_name || "", set_code: sc.set || "",
            artist: sc.artist || "", price_usd: sc.prices?.usd,
            is_foil: sc.finishes?.includes("foil") || false, printings: [],
...
export default function CartaDetailClient({ cardName }: { cardName: string }) {
    ...
    if (error || !card) {
        return (
            <div className="flex flex-col items-center justify-center gap-3 py-20">
                ...
                <Link href="/marketplace" className="text-xs font-semibold" style={{ color: "var(--accent)", textDecoration: "none" }}>
    ...
    return (
        <div style={{ maxWidth: 980, margin: "0 auto", padding: "16px" }}>
            <Link href="/marketplace" className="inline-flex items-center gap-1 mb-4 text-sm font-semibold" style={{ color: "var(--muted)", textDecoration: "none" }}>
                <ChevronLeft style={{ width: 16, height: 16 }} /> Volver al marketplace
            </Link>
            ...
            <div className="flex flex-col md:flex-row gap-6">
                <div className="shrink-0 mx-auto md:mx-0">
                    ...
                    <Link
                        ...
                        className="block mt-4 text-center text-xs font-semibold py-2.5 rounded-xl"
                        style={{ backgroundColor: "var(--accent)", color: "white", textDecoration: "none" }}
                    >
                        Ver vendedores en marketplace
                    </Link>
                </div>
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {card.mana_cost && (
                            <span className="text-xs font-semibold px-2 py-1 rounded" style={{ backgroundColor: "var(--surface)", color: "var(--foreground)" }}>{card.mana_cost}</span>
                        )}
                        {card.cmc > 0 && (
                            <span className="text-xs font-semibold px-2 py-1 rounded" style={{ backgroundColor: "var(--surface)", color: "var(--muted)" }}>CMC {card.cmc}</span>
                        )}
                        {card.colors.length > 0 && (
                            <span className="text-xs font-semibold px-2 py-1 rounded" style={{ backgroundColor: "var(--surface)", color: "var(--muted)" }}>{card.colors.join(", ")}</span>
                        )}
                        {card.power && card.toughness && (
                            <span className="text-xs font-bold px-2 py-1 rounded" style={{ backgroundColor: "var(--surface)", color: "var(--foreground)" }}>{card.power}/{card.toughness}</span>
                        )}
                        {card.loyalty && (
                            <span className="text-xs font-bold px-2 py-1 rounded" style={{ backgroundColor: "var(--surface)", color: "var(--foreground)" }}>Loyalty: {card.loyalty}</span>
                        )}
                        {card.rarity && (
                            <span className="text-xs font-semibold px-2 py-1 rounded uppercase" style={{ backgroundColor: "var(--surface)", color: rarityColor[card.rarity] ?? "var(--muted)" }}>{card.rarity}</span>
                        )}
                        {card.price_usd && (
                            <span className="text-xs font-bold px-2 py-1 rounded" style={{ backgroundColor: "var(--surface-solid)", border: "1px solid var(--border)", color: "var(--foreground)" }}>${card.price_usd} USD</span>
                        )}
                        {card.is_foil && (
                            <span className="text-[10px] font-bold px-2 py-1 rounded uppercase" style={{ backgroundColor: "rgba(202,138,4,0.1)", color: "var(--yellow)" }}>Foil</span>
                        )}
                    </div>

                    {card.oracle_text && (
                        <div className="p-4 rounded-xl border" style={{ backgroundColor: "var(--surface-solid)", borderColor: "var(--border)" }}>
                            <p className="text-sm leading-[22px] m-0 whitespace-pre-line" style={{ color: "var(--foreground)" }}>{card.oracle_text}</p>
                        </div>
                    )}

                    {card.flavor_text && (
                        <p className="text-sm italic m-0 leading-[20px] px-4" style={{ color: "var(--muted)", borderLeft: "2px solid var(--border)" }}>{card.flavor_text}</p>
                    )}

                    {card.keywords && card.keywords.length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                            {card.keywords.map((kw) => (
                                <span key={kw} className="text-[10px] font-semibold px-2 py-0.5 rounded" style={{ backgroundColor: "rgba(59,130,246,0.08)", color: "var(--accent)" }}>{kw}</span>
                            ))}
                        </div>
                    )}
                    ...
                    {card.printings.length > 1 && (
                        <div>
                            <span className="text-[11px] font-bold uppercase tracking-wider mb-2 block" style={{ color: "var(--muted)" }}>Ediciones ({card.printings.length})</span>
                            <div className="flex flex-wrap gap-1.5">
                    ...
                    {card.legality.length > 0 && (
                        <div>
                            <span className="text-[11px] font-bold uppercase tracking-wider mb-2 block" style={{ color: "var(--muted)" }}>Legalidad</span>
                            <div className="flex flex-wrap gap-1.5">
                                {card.legality
                                    .filter((l) => l.legality !== "not_legal" && l.legality !== "NOT_LEGAL")
                                    .map((l) => (
                                    <span key={l.format_slug} className="text-[10px] font-semibold px-2 py-1 rounded" style={{ backgroundColor: "var(--surface)", color: legalityColor[l.legality] ?? "var(--muted)" }}>
