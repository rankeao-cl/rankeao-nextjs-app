...
export async function getBadgeDetail(slug: string) {
    return apiFetch<ApiResponse<{ badge: Badge }>>(`/gamification/badges/${encodeURIComponent(slug)}`);
}

export async function getBadgeCategories() {
    return apiFetch<ApiResponse<{ categories: BadgeCategory[] }>>("/gamification/badge-categories", undefined, { revalidate: 300 });
}

// ── Cosmetics ──

export async function getCosmetics(params?: Params) {
    return apiFetch<ApiResponse<{ cosmetics: Cosmetic[] }>>("/gamification/cosmetics", params, { revalidate: 120 });
}

export async function getCosmeticDetail(slug: string) {
    return apiFetch<ApiResponse<{ cosmetic: Cosmetic }>>(`/gamification/cosmetics/${encodeURIComponent(slug)}`);
}

// ── Titles ──

export async function getTitles(params?: Params) {
    return apiFetch<ApiResponse<{ titles: Title[] }>>("/gamification/titles", params, { revalidate: 120 });
}

export async function getTitleDetail(slug: string) {
    return apiFetch<ApiResponse<{ title: Title }>>(`/gamification/titles/${encodeURIComponent(slug)}`);
}

// ── Seasons (Gamification) ──

export async function getGamificationSeasons() {
    return apiFetch<ApiResponse<{ seasons: GamificationSeason[] }>>("/gamification/seasons", undefined, { revalidate: 300 });
}

export async function getGamificationSeason(seasonId: string) {
    return apiFetch<ApiResponse<{ season: GamificationSeason }>>(`/gamification/seasons/${encodeURIComponent(seasonId)}`);
}

export async function getSeasonLeaderboard(seasonId: string, params?: Params) {
    return apiFetch<ApiResponse<{ entries: SeasonLeaderboardEntry[] }>>(`/gamification/seasons/${encodeURIComponent(seasonId)}/leaderboard`, params, { revalidate: 60 });
}

export async function getSeasonLeaderboardByGames(seasonId: string, params?: Params) {
    return apiFetch<ApiResponse<{ entries: SeasonLeaderboardEntry[] }>>(`/gamification/seasons/${encodeURIComponent(seasonId)}/leaderboard/games`, params, { revalidate: 60 });
}
...
export async function getXpEvents() {
    return apiFetch<ApiResponse<{ events: XpEvent[] }>>("/gamification/xp-events", undefined, { revalidate: 300 });
}

// ── My Cosmetics / Titles / XP / Equipped ──

export async function getMyCosmetics(token?: string) {
    return apiFetch<ApiResponse<{ cosmetics: Cosmetic[] }>>("/social/me/cosmetics", undefined, { cache: "no-store", token });
}

export async function getMyTitles(token?: string) {
    return apiFetch<ApiResponse<{ titles: Title[] }>>("/social/me/titles", undefined, { cache: "no-store", token });
}
...
export async function updateEquipped(payload: { title_id?: string; cosmetic_ids?: string[] }, token?: string) {
    return apiPatch<ApiResponse<ApiMessage>>("/social/me/equipped", payload, { token });
}
