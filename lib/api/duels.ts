...
export async function getDuels(params?: Record<string, string | number | boolean | undefined>, token?: string) {
    const res = await apiFetch<ApiResponse<{ duels: DuelInfoRaw[] }>>("/social/duels", params, { cache: "no-store", token });
...
export async function getDuel(duelId: string, token?: string) {
    const res = await apiFetch<ApiResponse<{ duel: DuelInfoRaw }>>(`/social/duels/${encodeURIComponent(duelId)}`, undefined, { cache: "no-store", token });
...
export async function createDuel(data: CreateDuelRequest, token?: string) {
    const res = await apiPost<ApiResponse<{ duel: DuelInfoRaw }>>("/social/duels", {
...
export async function acceptDuel(duelId: string, token?: string) {
    return apiPost<ApiResponse<ApiMessage>>(`/social/duels/${encodeURIComponent(duelId)}/accept`, {}, { token });
}

export async function declineDuel(duelId: string, token?: string) {
    return apiPost<ApiResponse<ApiMessage>>(`/social/duels/${encodeURIComponent(duelId)}/decline`, {}, { token });
}

export async function cancelDuel(duelId: string, token?: string) {
    return apiPost<ApiResponse<ApiMessage>>(`/social/duels/${encodeURIComponent(duelId)}/cancel`, {}, { token });
}

// ── Report / Confirm / Dispute ──

export async function reportDuelResult(duelId: string, data: ReportDuelResultRequest, token?: string) {
    return apiPost<ApiResponse<ApiMessage>>(`/social/duels/${encodeURIComponent(duelId)}/report`, data, { token });
}

export async function confirmDuelResult(duelId: string, token?: string) {
    return apiPost<ApiResponse<ApiMessage>>(`/social/duels/${encodeURIComponent(duelId)}/confirm`, {}, { token });
}
...
export async function getDuelComments(duelId: string, token?: string) {
    return apiFetch<ApiResponse<{ comments: DuelComment[] }>>(`/social/duels/${encodeURIComponent(duelId)}/comments`, undefined, { cache: "no-store", token });
}

export async function createDuelComment(duelId: string, data: { content: string }, token?: string) {
    return apiPost<ApiResponse<{ comment: DuelComment }>>(`/social/duels/${encodeURIComponent(duelId)}/comments`, data, { token });
}

// ── Report opponent ──

export async function reportDuelOpponent(duelId: string, data: { reason: string }, token?: string) {
    return apiPost<ApiResponse<ApiMessage>>(`/social/duels/${encodeURIComponent(duelId)}/report-opponent`, data, { token });
}
...
export async function broadcastDuelSearch(data: {
    ...
    return apiPost<ApiResponse<ApiMessage>>("/social/duels/search", data, { token });
}
